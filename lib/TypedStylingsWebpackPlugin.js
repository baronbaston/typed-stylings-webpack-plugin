...
var TypedStylingsWebpackPlugin = /** @class */ (function () {
    ...
    TypedStylingsWebpackPlugin.prototype.cleanupTypings = function () {
        var _this = this;
        var sliceLength = '.d.ts'.length;
        this.includePaths
            .map(function (dir) { return _this.walkSync(dir); })
            .reduce(function (result, dirFiles) { return result.concat(dirFiles); }, [])
            .filter(function (file) { return _this.preProcessors.some(function (preProcessor) { return preProcessor.testTyping(file.path) && !fs.existsSync(file.path.slice(0, file.path.length - sliceLength)); }); })
